...
import {
  Activity,
  ActivityDashboardParams,
  DashboardActivitiesAllData,
  DashboardActivityData,
  FetchActivityParams,
  ModifiedActivity,
} from "../../utilities/common/exportDataTypes/activityDatatypes";
...
type InitialState = {
  ...
  error: string;
  addActivityLoader: boolean;
  getActivityLoader: boolean;
  dashboardActivitiesAllData: DashboardActivitiesAllData;
  dashboardActivitiesLoader: boolean;
  editable: boolean;
  dashboardActivityData: DashboardActivityData;
...
export const fetchAllActivitiesByModuleId = createAsyncThunk(
  "activity/fetchAllActivitiesByModuleId",
  async ({
    moduleName,
    moduleId,
    params,
    ...
    try {
      ...
      const response = await axios.post(
        `${baseUrl}activity/${moduleName}/${moduleId}?page=${params?.page}&limit=${params?.limit}&search=${params?.search}`,
        null,
        config
      );
...
export const fetchAllActivities = createAsyncThunk(
  ...
  async (params: FetchActivityParams) => {
    try {
      const config = {
        headers: {
          Authorization: `Bearer ${
      ...
      const response = await api.post(
        `activity/getActivities?page=${params?.page}&limit=${params?.limit}&search=${params?.search}&view=${params?.view}`,
        {},
        config
      );
...
export const fetchAllActivitiesWithoutParams = createAsyncThunk(
  "activity/fetchAllActivities",
  async () => {
    try {
      const config = {
  ...
  async (payload: Activity) => {
    try {
      ...
      const response = await axios.put(
        `${baseUrl}Activity/${payload?.activityId}`,
        payloadWithoutCreatedAt,
        config
      );
      if (response.status === 200) {
...
export const deleteActivitiesById = createAsyncThunk(
  "activities/deleteActivitiesById",
  async (activityId: React.Key[]) => {
    try {
      const config = {
        headers: {
          Authorization: `Bearer ${
...
export const createAndGetAllActivities =
  (payload: Activity, params: FetchActivityParams) =>
  async (dispatch: AppDispatch) => {
    await dispatch(addActivity(payload));
    await dispatch(fetchAllActivities(params));
  };

export const updateActivityByIdAndGetAllActivities =
  (payload: Activity, params: FetchActivityParams) =>
  async (dispatch: AppDispatch) => {
...
export const deleteActivitysByIdAndGetAllActivitiesTotal =
  (activityId: React.Key[], params: FetchActivityParams) =>
  async (dispatch: AppDispatch) => {
...
export const createAndGetAllActivitiesByModuleId =
  (
    payload: Activity,
    params: FetchActivityParams,
    moduleName: string,
  ...
  async (dispatch: AppDispatch) => {
    await dispatch(addActivity(payload));
    await dispatch(
      fetchAllActivitiesByModuleId({ moduleName, moduleId, params })
    );
  };

export const updateActivityByIdAndGetAllActivitiesByOpportunityId =
  (
    payload: Activity,
    params: FetchActivityParams,
    moduleName: string,
...
export const deleteActivitysByIdAndGetAllActivities =
  (
...
export const fetchAllDashboardActivities = createAsyncThunk(
  "activities/fetchAllDashboardActivities",
  async (params: ActivityDashboardParams) => {
    try {
      const config = {
        headers: {
          Authorization: `Bearer ${
      ...
      const response = await api.post(
        `dashboard/activity-all?page=${params?.page}&limit=${params?.limit}&search=${params?.search}`,
        params?.filterParams,
        config
      );
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchAllDashboardActivities.fulfilled, (state, action) => {
      ...
      if (action?.payload?.data) {
        ...
        const actualdata = action.payload?.data?.activity_data?.data?.map(
          (item: ModifiedActivity) => {
            return {
              ...item,
              company: item?.company?.accountId,
