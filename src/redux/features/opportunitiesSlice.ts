...
import {
  ...
  DashboardOpportunitiesParams,
  DashboardOpportunityData,
  FetchOpportunitiesParams,
  ModifiledOpportunity,
  Opportunity,
  Pagination,
} from "../../utilities/common/exportDataTypes/opportunityDataTypes";
...
type InitialState = {
  ...
  error: string;
  addOpportunityLoader: boolean;
  getOpportunityLoader: boolean;
  editable: boolean;
  dashboardOpportunitiesAllData: DashboardOpportunitiesAllData;
  dashboardOpportunitiesLoader: boolean;
  kanbanLoader: boolean;
  dashboardOpportunitiesData: DashboardOpportunityData;
  isModalOpenOpportunity: boolean;
...
export const fetchAllOpportunities = createAsyncThunk(
  "opportunities/fetchUsers",
  async (params: FetchOpportunitiesParams) => {
    try {
      const config = {
        headers: {
          Authorization: `Bearer ${
      ...
      const response = await api.post(
        `opportunity/getAllopportunity?page=${params?.page}&limit=${params?.limit}&search=${params?.search}&view=${params?.view}`,
        {},
        config
      );
  ...
  async (payload: Opportunity) => {
    try {
      ...
      const response = await axios.post(
        `${baseUrl}opportunity/create-opportunity`,
...
export const getOpportunityById = createAsyncThunk(
  "opportunities/getOpportunityById",
  async (opportunityId: string) => {
    try {
      ...
      const response = await axios.get(
        `${baseUrl}opportunity/${opportunityId}`,
        config
      );
      if (response.status === 200) {
        console.log("Success", response?.data);
...
export const updateOpportunityById = createAsyncThunk(
  ...
  async (payload: Opportunity) => {
    try {
      ...
      const response = await axios.put(
        `${baseUrl}opportunity/${payload?.opportunityId}`,
        payloadWithoutCreatedAt,
        config
      );
      if (response.status === 200) {
...
export const updateOpportunityByIdForOwner = createAsyncThunk(
  "opportunities/updateOpportunityByIdForOwner",
  async (payload: Opportunity) => {
    try {
      const config = {
        headers: {
...
export const deleteOpportunityById = createAsyncThunk(
  "opportunities/deleteOpportunityById",
  async (opportunityId: React.Key[], { rejectWithValue }) => {
    try {
      const config = {
        headers: {
          Authorization: `Bearer ${
      ...
      const response = await axios.post(
        `${baseUrl}opportunity/bulk-delete`,
        payload,
        config
      );
      if (response.status === 200) {
...
export const updateOpportunityByIdAndGetAllOpportunitiesWithParams =
  (payload: Opportunity, params: FetchOpportunitiesParams) => async (dispatch: AppDispatch) => {
    await dispatch(updateOpportunityById(payload));
    await dispatch(fetchAllOpportunities(params));
  };

export const createAndGetAllOpportunities =
  (payload: Opportunity, params: FetchOpportunitiesParams) =>
...
export const updateOpportunityByIdAndGetAllOpportunities =
  (payload: Opportunity, params: FetchOpportunitiesParams) =>
  async (dispatch: AppDispatch) => {
    await dispatch(updateOpportunityById(payload));
    await dispatch(fetchAllOpportunities(params));
...
export const deleteOpportunityByIdAndGetAllOpportunities =
  (opportunityId: React.Key[], params: FetchOpportunitiesParams) =>
  async (dispatch: AppDispatch) => {
...
export const updateOpportunityByIdAndGetAudits =
...
export const updateOpportunityForOwnerByIdAndGetAudits =
  (payload: any) => async (dispatch: AppDispatch) => {
...
export const fetchAllDashboardOpportunities = createAsyncThunk(
  "opportunities/fetchAllDashboardOpportunities",
  async (params: DashboardOpportunitiesParams) => {
    try {
      const config = {
        headers: {
          Authorization: `Bearer ${
      ...
      const response = await api.post(
        `dashboard/opportunity-all?page=${params?.page}&limit=${params?.limit}&search=${params?.search}&stage=${params?.stage}&currency=${params?.currency}`,
        params?.filterParams,
        config
      );
...
export const fetchAllOpportunitiesByModuleId = createAsyncThunk(
  "opportunities/fetchAllOpportunitiesByModuleId",
  async ({
    moduleName,
    moduleId,
    params,
    ...
    try {
      ...
      const response = await api.post(
        `opportunity/getAllopportunity/by${moduleName}/${moduleId}?page=${
          params?.page
        }&limit=${params?.limit}&search=${params?.search?.toLocaleLowerCase()}`,
        {},
        config
      );
...
export const createAndGetAllOpportunitiesByModuleId =
  (
    payload: Opportunity,
    params: FetchOpportunitiesParams,
    moduleName: string,
  ...
  async (dispatch: AppDispatch) => {
    await dispatch(addOpportunity(payload));
    await dispatch(
      fetchAllOpportunitiesByModuleId({ moduleName, moduleId, params })
    );
  };

export const deleteOpportunitiesByIdAndGetAllOpportunitiesByModuleId =
  (
    opportunityId: React.Key[],
    params: FetchOpportunitiesParams,
    moduleName: string,
    moduleId: string
  ) =>
...
export const opportunitySlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(
      fetchAllDashboardOpportunities.fulfilled,
      (state, action) => {
        ...
        if (action?.payload?.data?.opportunity_data?.data) {
          const actualdata = action.payload?.data?.opportunity_data?.data?.map(
            (item: ModifiledOpportunity) => {
              return {
                ...item,
                company:
    ...
    builder.addCase(
      fetchAllDashboardOpportunities.rejected,
      (state, action) => {
        state.dashboardOpportunitiesLoader = false;
        state.dashboardOpportunitiesAllData = emptyDashboardOpportunities;
        state.error = action.error.message || "Operation cannot be completed, you may want to contact administrator for help.";
    ...
    builder.addCase(
      fetchAllOpportunitiesByModuleId.fulfilled,
      (state, action) => {
    ...
    builder.addCase(
      fetchAllOpportunitiesByModuleId.rejected,
      (state, action) => {
        state.kanbanLoader = false;
        state.getOpportunityLoader = false;
        state.opportunities = [];
...
export const {
  ...
  resetOpportunity,
  handleOpportunityChangeReducer,
  setCompanyId,
  resetOpportunities,
  setEditableMode,
