...
import {
  ...
  ModifiedContact,
  Pagination,
} from "../../utilities/common/exportDataTypes/contactsDataTypes";
...
type InitialState = {
  ...
  error: string;
  addContactLoader: boolean;
  getContactLoader: boolean;
  editable: boolean;
  isModalOpenContact: boolean;
  contactForLookup: Contact;
  bulkContactLoader: boolean;
  ...
  async (params: FetchContactsParams) => {
    try {
      ...
      const response = await api.post(
        `contact/getContacts?page=${params?.page}&limit=${params?.limit}&search=${params?.search}&view=${params?.view}`,
        {},
        config
      );
...
export const fetchAllContactsWithoutParams = createAsyncThunk(
  "contacts/fetchAllContactsWithoutParams",
  async () => {
    try {
      const config = {
        headers: {
  ...
  async (payload: Contact) => {
    try {
      ...
      const response = await axios.put(
        `${baseUrl}contact/${payload?.contactId}`,
        payloadWithoutCreatedAt,
        config
      );
      if (response.status === 200) {
...
export const deleteContactsById = createAsyncThunk(
  "contact/deleteContactsById",
  async (contactId: React.Key[]) => {
    try {
      const config = {
        headers: {
          Authorization: `Bearer ${
...
export const fetchAllContactsByModuleId = createAsyncThunk(
  "contacts/fetchAllContactsByModuleId",
  async ({
    moduleName,
    moduleId,
    params,
    ...
    try {
      ...
      const response = await axios.post(
        `${baseUrl}contact/${moduleName}/${moduleId}?page=${
          params?.page
...
export const addBulkContacts = createAsyncThunk(
  ...
  async (file: File, thunkAPI) => {
    try {
      ...
      const response = await axios.post(
        `${baseUrl}contact/upload-csv-contacts`,
        formData,
        config
      );
      if (response?.status === 200) {
...
export const createAndGetAllContacts =
  (payload: Contact, params: FetchContactsParams) =>
...
export const updateContactByIdAndGetThatContactById =
  (payload: Contact) => async (dispatch: AppDispatch) => {
    await dispatch(updateContactById(payload));
    await dispatch(getContactById(payload?.contactId));
  };

export const deleteContactsByIdAndGetAllContacts =
  (contactId: React.Key[], params: FetchContactsParams) =>
  async (dispatch: AppDispatch) => {
...
export const createAndGetAllContactsByModuleId =
  (
    payload: Contact,
    params: FetchContactsParams,
    moduleName: string,
  ...
  async (dispatch: AppDispatch) => {
    await dispatch(addContact(payload));
    await dispatch(
      fetchAllContactsByModuleId({ moduleName, moduleId, params })
    );
  };

export const deleteContactsByIdAndGetAllContactsByModuleId =
  (
    contactId: React.Key[],
    params: FetchContactsParams,
    moduleName: string,
...
// AUDITS
export const updateContactByIdAndGetAudits =
...
// FOR LOOKUP
export const createAndGetAllContactsWithoutParams =
  (payload: Contact) => async (dispatch: AppDispatch) => {
    await dispatch(addContact(payload));
...
export const contactSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(
      fetchAllContactsWithoutParams.fulfilled,
      (state, action) => {
        state.loading = false;
        state.error = "";
