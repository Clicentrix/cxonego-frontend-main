...
function isEmailId(str: string | null) {
  return typeof str === "string" && str !== null;
}
...
type InitialState = {
  ...
  loading: boolean;
  userFormPage: number;
  inviteTeamMatesLoader: boolean;
  profileRoute: boolean;
  updateUserLoader: boolean;
  getUserLoader: boolean;
  invitedUsers: InvitedUser[];
  subscription: Subscription;
  updateUserRoleLoader: boolean;
  userOnboardStatusData: User;
  onboardingStatusLoader: boolean;
};
...
export const sendForgetPasswordEmail = createAsyncThunk(
  "authentication/sendForgetPasswordEmail",
  async (email: string) => {
    const actionCodeSettings = {
      url: "https://cxonego-frontend.vercel.app/", // Specify the desired redirect URL
...
export const getUserByIdParam = createAsyncThunk(
  "authentication/getUserByIdParam",
  async (userId: string) => {
    try {
      const config = {
        headers: {
...
export const addUser = createAsyncThunk(
  ...
  async (payload: FirebasePostUser) => {
    try {
      const config = {
        headers: {
          Authorization: `Bearer ${
            payload?.accessToken ? payload?.accessToken : undefined
          }`,
        },
      };
...
export const updateUserProfileById = createAsyncThunk(
  "authentication/updateUserProfileById",
  async (payload: ShortUserProfile) => {
    try {
      const config = {
        headers: {
          Authorization: `Bearer ${
      ...
      const response = await axios.post(
        `${baseUrl}users/updateUserProfile/${payload?.userId}`,
        payload,
        config
      );
      if (response.status === 200) {
...
export const updateWebFcmUserPatch = createAsyncThunk(
  "authentication/updateWebFcmUserPatch",
  async (fcmToken: string) => {
    try {
      ...
      const response = await axios.patch(
        `${baseUrl}users/update/${userIdHere}`,
        payload
      );
      if (response.status === 200) {
        console.log("Success", response?.data);
...
export const updateAndroidFcmUserPatch = createAsyncThunk(
  "authentication/updateAndroidFcmUserPatch",
  async (fcmToken: string) => {
    try {
      // const fcmTokenHere = isFcmToken(fcmToken) ? fcmToken : undefined;
      const payload = {
...
export const updateUserByUserIdExceptAdmin = createAsyncThunk(
  "authentication/updateUserByUserIdExceptAdmin",
  async (payload: UserUpdateType) => {
    try {
      const config = {
        headers: {
          Authorization: `Bearer ${
...
// InviteTeamMate flow
export const inviteTeammates = createAsyncThunk(
  "authentication/inviteTeammates",
...
export const inviteTeammatesLoggedIn = createAsyncThunk(
  "authentication/inviteTeammatesLoggedIn",
  async (payload: InviteUserPayload) => {
    try {
      const config = {
        headers: {
...
export const updateUserRoleByAdmin = createAsyncThunk(
  "authentication/updateUserRoleByAdmin",
  async (payload: ChangeUserRole) => {
    try {
      const config = {
        headers: {
          Authorization: `Bearer ${
...
export const verifyRecaptcha = createAsyncThunk(
  "authentication/verifyRecaptcha",
  async (payload: any) => {
    try {
      const config = {
        headers: {
...
export const blockUserByAdmin = createAsyncThunk(
  "authentication/blockUserByAdmin",
...
export const deleteUserFromInvitedArray = createAsyncThunk(
  "authentication/deleteUserFromInvitedArray",
  async (payload: { adminId: string; userEmail: string }) => {
    try {
      const config = {
        headers: {
          Authorization: `Bearer ${
...
export const checkInvitationValidity = createAsyncThunk<
  ApiResponseType,
  { organizationId: string; userEmail: string },
  { rejectValue: ApiErrorType }
>(
  "authentication/checkInvitationValidity",
  async (
    payload: { organizationId: string; userEmail: string },
    { rejectWithValue }
  ) => {
    try {
      const config = {
...
export const inviteTeammatesLoggedInAndGetUserDetails =
  (payload: InviteUserPayload) => async (dispatch: AppDispatch) => {
    await dispatch(inviteTeammatesLoggedIn(payload));
    await dispatch(getUserById());
  };

export const blockUserAndByIdAndGetUserById =
  (payload: BlockUserPayload) => async (dispatch: AppDispatch) => {
...
export const checkUserOnBoardingStatus = createAsyncThunk(
  "authentication/checkUserOnBoardingStatus",
  async (email: string) => {
    try {
      const config = {
        headers: {
...
const authenticationSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(getUserById.fulfilled, (state, action) => {
      ...
      if (action?.payload?.data) {
        ...
        if (action?.payload?.data?.invitedUsers?.length > 0) {
          state.invitedUsers = action?.payload?.data?.invitedUsers?.map(
            (item: InvitedUser, index: number) => {
              return { ...item, userId: index + 1, isBlocked: item?.isBlocked };
            }
          );
        }
        if (action?.payload?.data?.organisation?.subscriptions?.length > 0) {
          const subscriptionHere =
            action?.payload?.data?.organisation?.subscriptions[0];
          state.subscription = {
            ...subscriptionHere,
    ...
    builder.addCase(checkInvitationValidity.rejected, (state, action) => {
      ...
      if (action.payload) message.error(action.payload?.error.message);
    });
    ...
    builder.addCase(checkUserOnBoardingStatus.fulfilled, (state, action) => {
      ...
      if (action?.payload?.data) {
        state.userOnboardStatusData = {
          ...action.payload?.data,
          role: action?.payload?.data?.invitedUsers?.find(
