...
//for export data.
export const getAllAppointments = createAsyncThunk("accounts/getAllAppointments", async () => {
    try {
        const config = {
            headers: {
...
export const fetchAllAppointments = createAsyncThunk("appointments/fetchAllAppointments", async () => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
        ...
        const response = await axios.get(`${baseUrl}calender/getAppointments`, config);
        if (response.status === 200) {
            console.log("Success", response?.data);
        }
        return response.data;
...
export const fetchAllOrgsPeople = createAsyncThunk("appointments/fetchAllOrgsPeopple", async () => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
...
export const addAppointment = createAsyncThunk("appointments/addAppointment", async (payload) => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
        ...
        const response = await axios.post(`${baseUrl}calender/create-calender`, payloadWithoutCreatedAt, config);
        if (response.status === 200) {
            console.log("Success", response?.data);
        }
        return response.data;
...
export const getAppointmentById = createAsyncThunk("appointments/getAppointmentById", async (appointmentId) => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
        ...
        const response = await axios.get(`${baseUrl}calender/${appointmentId}`, config);
        if (response.status === 200) {
            console.log("Success", response?.data);
        }
        return response.data;
...
export const updateAppointmentById = createAsyncThunk("appointments/updateAppointmentById", async (payload) => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
        ...
        const { createdAt, updatedAt, ...payloadWithoutCreatedAt } = payload;
        const response = await axios.put(`${baseUrl}calender/update-calender/${payload?.appointmentId}`, payloadWithoutCreatedAt, config);
        if (response.status === 200) {
            console.log("Success", response?.data);
        }
        return response.data;
...
export const deleteAppointmentsById = createAsyncThunk("appointments/deleteAppointmentsById", async (appointmentId) => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
        ...
        const response = await axios.delete(`${baseUrl}calender/delete-calender/${appointmentId}`, config);
        if (response.status === 200) {
            console.log("Success", response?.data);
        }
        return response.data;
...
// For All Appointment
export const createAndGetAllAppointments = (payload) => async (dispatch) => {
    await dispatch(addAppointment(payload));
    await dispatch(fetchAllAppointments());
};
export const updateAppointmentByIdAndGetAllAppointments = (payload) => async (dispatch) => {
    await dispatch(updateAppointmentById(payload));
    await dispatch(fetchAllAppointments());
};
export const deleteAppointsByIdAndGetAllAppointmentsTotal = (appointmentId) => async (dispatch) => {
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(getAppointmentById.fulfilled, (state, action) => {
            if (action.payload.data) {
                state.appointment = {
                    ...action?.payload?.data,
                    startDateTime: dayjs(action?.payload?.data.startDateTime),
                    endDateTime: dayjs(action?.payload?.data.endDateTime),
                };
            }
            ...
            if (action?.payload?.data) {
                state.appointments = action?.payload?.data.map((item) => {
                    return {
                        ...
                        participants: item?.calparticipaters?.map((p) => p.email).join(", ") || null
                    };
                });
            }
            else {
...
export const { handleInputChangeReducerAppointment, resetAppointment, resetAppointments, setEditableMode, } = appointmentSlice.actions;
