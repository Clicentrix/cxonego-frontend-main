...
// Fetch Leads data
export const getAllLeads = createAsyncThunk("accounts/getAllLeads", async () => {
    try {
        const config = {
            headers: {
...
export const fetchAllLeadsWithParams = createAsyncThunk("leads/fetchAllLeadsWithParams", async (params) => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
        ...
        const response = await api.post(`lead/get-leads?page=${params?.page}&limit=${params?.limit}&search=${params?.search}&view=${params?.view}`, {}, config);
        if (response.status === 200) {
            console.log("Success", response?.data);
        }
        return response.data;
...
export const addLead = createAsyncThunk("leads/addLead", async (payload) => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
        ...
        const response = await axios.post(`${baseUrl}lead/create-lead`, payloadWithoutCreatedAt, config);
        if (response.status === 200) {
            console.log("Success", response?.data);
        }
        return response.data;
...
export const getLeadById = createAsyncThunk("leads/getLeadById", async (leadId) => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
        ...
        const response = await axios.get(`${baseUrl}lead/get-lead/${leadId}`, config);
        if (response.status === 200) {
            console.log("Success", response?.data);
        }
        return response.data;
...
export const updateLeadById = createAsyncThunk("leads/updateLeadById", async (payload) => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
        ...
        const response = await axios.put(`${baseUrl}lead/update-lead/${payload?.leadId}`, payloadWithoutCreatedAt, config);
        if (response.status === 200) {
            console.log("Success", response?.data);
        }
        return response.data;
...
export const updateLeadByIdForOwner = createAsyncThunk("leads/updateLeadByIdForOwner", async (payload) => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
...
export const deleteLeadById = createAsyncThunk("leads/deleteLeadById", async (leadId) => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
        ...
        const response = await axios.post(`${baseUrl}lead/bulk-delete`, payload, config);
        if (response.status === 200) {
            console.log("Success", response?.data);
        }
        return response.data;
...
export const addBulkLeads = createAsyncThunk("user/createBulkUsers", async (file) => {
    try {
        const config = {
        ...
        const response = await axios.post(`${baseUrl}lead/upload`, formData, config);
        if (response?.status === 200) {
...
// For related View
export const fetchAllLeadsByModuleId = createAsyncThunk("leads/fetchAllLeadsByModuleId", async ({ moduleName, moduleId, params, }) => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
        ...
        const response = await api.post(`lead/by${moduleName}/${moduleId}?page=${params?.page}&limit=${params?.limit}&search=${params?.search?.toLocaleLowerCase()}`, {}, config);
        if (response.status === 200) {
            console.log("Success", response?.data);
        }
        return response.data;
...
export const createAndGetAllleads = (payload, params) => async (dispatch) => {
    await dispatch(addLead(payload));
    await dispatch(fetchAllLeadsWithParams(params));
    await dispatch(resetAccountForLookup());
};
export const updateLeadByIdAndGetAllleads = (payload, params) => async (dispatch) => {
    await dispatch(updateLeadById(payload));
    await dispatch(fetchAllLeadsWithParams(params));
};
// Define updateLeadByIdAndGetAllleadsWithoutParams properly
export const updateLeadByIdAndGetAllleadsWithParams = (payload, params) => async (dispatch) => {
    // Perform asynchronous operations such as API calls here
    await dispatch(updateLeadById(payload)); // Example API call
    await dispatch(fetchAllLeadsWithParams(params));
};
export const deleteLeadByIdAndGetAllleads = (leadId, params) => async (dispatch) => {
    await dispatch(deleteLeadById(leadId));
    await dispatch(fetchAllLeadsWithParams(params));
};
export const addBulkLeadsAndGetAllleads = (file, params) => async (dispatch) => {
    await dispatch(addBulkLeads(file));
    await dispatch(fetchAllLeadsWithParams(params));
};
// For related View
export const createAndGetAllLeadsByModuleId = (payload, params, moduleName, moduleId) => async (dispatch) => {
    await dispatch(addLead(payload));
    await dispatch(fetchAllLeadsByModuleId({ moduleName, moduleId, params }));
};
export const deleteLeadsByIdAndGetAllLeadsByModuleId = (leadId, params, moduleName, moduleId) => async (dispatch) => {
    await dispatch(deleteLeadById(leadId));
    await dispatch(fetchAllLeadsByModuleId({ moduleName, moduleId, params }));
};
// AUDIT
export const updateLeadByIdAndGetAudits = (payload) => async (dispatch) => {
    await dispatch(updateLeadById(payload));
...
export const updateLeadOwnerByIdAndGetAudits = (payload) => async (dispatch) => {
...
export const fetchAllDashboardLeads = createAsyncThunk("leads/fetchAllDashboardLeads", async (params) => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
        ...
        const response = await api.post(`dashboard/leads-all?page=${params?.page}&limit=${params?.limit}&search=${params?.search}&status=${params?.status}`, params?.filterParams, config);
        if (response.status === 200) {
            console.log("Success", response?.data);
        }
        return response.data;
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(fetchAllDashboardLeads.fulfilled, (state, action) => {
            ...
            if (action?.payload?.data?.lead_data?.data) {
                const actualdata = action.payload?.data?.lead_data?.data?.map((item) => {
                    return {
                        ...item,
                        company: item?.company?.accountId === null ||
                            item?.company?.accountId === "" ||
...
export const { handleInputChangeReducerLead, resetLead, handleLeadChangeReducer, setCompanyId, resetLeads, setEditableMode, } = leadSlice.actions;
