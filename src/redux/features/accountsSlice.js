...
export const fetchAllAccounts = createAsyncThunk("accounts/fetchAllAccounts", async (params) => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
        ...
        const response = await api.post(`account/getAccounts?page=${params?.page}&limit=${params?.limit}&search=${params?.search}&view=${params?.view}`, {}, config);
        if (response.status === 200) {
            console.log("Success", response?.data);
        }
        return response.data;
...
export const fetchAllAccountsWithoutParams = createAsyncThunk("accounts/fetchAllAccountsWithoutParams", async () => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
...
export const addAccount = createAsyncThunk("accounts/addAccount", async (payload) => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
        ...
        const response = await axios.post(`${baseUrl}account/`, payloadWithoutCreatedAt, config);
        if (response.status === 200) {
...
export const getAccountById = createAsyncThunk("accounts/getAccountById", async (accountId) => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
        ...
        const response = await axios.get(`${baseUrl}account/${accountId}`, config);
        if (response.status === 200) {
            console.log("Success", response?.data);
        }
        return response.data;
...
export const updateAccountById = createAsyncThunk("accounts/updateBAccountById", async (payload) => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
        ...
        const { createdAt, updatedAt, ...payloadWithoutCreatedAt } = payload;
        const response = await axios.put(`${baseUrl}account/${payload?.accountId}`, payloadWithoutCreatedAt, config);
        if (response.status === 200) {
            console.log("Success", response?.data);
        }
        return response.data;
...
export const deleteAccountsById = createAsyncThunk("accounts/deleteAccountsById", async (accountId) => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
        ...
        const response = await axios.post(`${baseUrl}account/bulk-delete`, payload, config);
        if (response.status === 200) {
            console.log("Success", response?.data);
        }
        return response.data;
...
export const createAndGetAllAccounts = (payload, params) => async (dispatch) => {
    await dispatch(addAccount(payload));
    await dispatch(fetchAllAccounts(params));
};
export const updateAccountByIdAndGetThatAccountById = (payload) => async (dispatch) => {
    await dispatch(updateAccountById(payload));
    await dispatch(getAccountById(payload?.accountId));
};
export const deleteAccountsByIdAndGetAllAccounts = (accountId, params) => async (dispatch) => {
    await dispatch(deleteAccountsById(accountId));
    await dispatch(fetchAllAccounts(params));
};
// User onboarding flow
export const addAccountAndUpdateUserData = (account, user) => async (dispatch) => {
    await dispatch(addAccount(account));
    await dispatch(updateUserByUserId(user));
};
export const updateAccountByIdAndGetAudits = (payload) => async (dispatch) => {
    await dispatch(updateAccountById(payload));
    await dispatch(getAccountById(payload?.accountId));
...
// FOR LOOKUP
export const createAndGetAllAccountsWithoutParams = (payload) => async (dispatch) => {
    await dispatch(addAccount(payload));
...
export const accountSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(fetchAllAccountsWithoutParams.fulfilled, (state, action) => {
            state.loading = false;
            state.error = "";
            if (action?.payload?.data) {
                state.totalAccounts = action.payload.data?.total;
...
export const { handleInputChangeReducerAccount, resetAccount, handleAccountChangeReducer, setEditableMode, 
// handleAddressInputChangeReducer,
resetAccounts, resetIsModalOpenAccount, resetAccountForLookup, } = accountSlice.actions;
