...
export const getAllReferrals = createAsyncThunk("accounts/getAllReferrals", async () => {
    try {
        const config = {
            headers: {
...
// Fetch Referral data
export const fetchAllReferrals = createAsyncThunk("referral/fetchAllReferralsByModuleId", async (params) => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
        ...
        const response = await api.post(`refer/getAllRefer/?page=${params?.page}&limit=${params?.limit}&search=${params?.search}`, {}, config);
        if (response.status === 200) {
            console.log("Success", response?.data);
        }
        return response.data;
...
export const addReferral = createAsyncThunk("referral/addReferral", async (payload) => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
        ...
        const response = await axios.post(`${baseUrl}/refer/create-refer`, payloadWithoutCreatedAt, config);
        if (response.status === 200) {
            console.log("Success", response?.data);
        }
        return response.data;
...
export const getReferralById = createAsyncThunk("referral/getReferralById", async (referId) => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
...
export const updateReferralById = createAsyncThunk("referral/updateReferralById", async (payload) => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
        ...
        const response = await axios.put(`${baseUrl}refer/update-refer/${payload?.referId}`, payloadWithoutCreatedAt, config);
        if (response.status === 200) {
            console.log("Success", response?.data);
        }
        return response.data;
...
export const deleteRefarralsById = createAsyncThunk("referral/deleteRefarralsById", async (referId) => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
        ...
        const response = await axios.post(`${baseUrl}refer/bulk-delete/`, payload, config);
        if (response.status === 200) {
            console.log("Success", response?.data);
        }
        return response.data;
...
// For All Referrals
export const createAndGetAllReferrals = (payload, params) => async (dispatch) => {
    await dispatch(addReferral(payload));
    await dispatch(fetchAllReferrals(params));
};
export const updateReferralByIdAndGetAllReferrals = (payload, params) => async (dispatch) => {
    await dispatch(updateReferralById(payload));
    await dispatch(fetchAllReferrals(params));
};
export const deleteReferralsByIdAndGetAllReferralsTotal = (referIds, params) => async (dispatch) => {
...
export const { handleInputChangeReducerReferral, resetReferral, resetReferrals, setEditableMode, setScreenWidth, } = referralSlice.actions;
