...
//   return typeof str === "string" && str !== null;
// }
export function checkAuthTokenAndLogoff(error) {
...
// Fetch Notes data
export const getAllNotes = createAsyncThunk("accounts/getAllNotes", async () => {
    try {
        const config = {
            headers: {
...
export const fetchAllNotes = createAsyncThunk("note/fetchAllNotes", async (params) => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
        ...
        const response = await api.post(`note/getAllNotes?page=${params?.page}&limit=${params?.limit}&search=${params?.search}`, {}, config);
        if (response.status === 200) {
            console.log("Success", response?.data);
        }
        return response.data;
...
export const addNote = createAsyncThunk("note/addNote", async (payload) => {
    try {
        ...
        const response = await axios.post(`${baseUrl}note/`, payloadWithoutCreatedAt, config);
        if (response.status === 200) {
            console.log("Success", response?.data);
        }
        return response.data;
...
export const updateNoteById = createAsyncThunk("note/updateNoteById", async (payload) => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
        ...
        const response = await axios.put(`${baseUrl}note/${payload?.noteId}`, payloadWithoutCreatedAt, config);
        if (response.status === 200) {
            console.log("Success", response?.data);
        }
        return response.data;
...
export const deleteNotesById = createAsyncThunk("note/deleteNotesById", async (noteIds) => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
        ...
        const response = await axios.post(`${baseUrl}note/bulk-delete`, payload, config);
        if (response.status === 200) {
            console.log("Success", response?.data);
        }
        return response.data;
...
// For related View
export const fetchAllNotesByModuleId = createAsyncThunk("notes/fetchAllNotesByModuleId", async ({ moduleName, moduleId, params, }) => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
        ...
        const response = await axios.post(`${baseUrl}note/${moduleName}/${moduleId}?page=${params?.page}&limit=${params?.limit}&search=${params?.search?.toLocaleLowerCase()}`, null, config);
        if (response.status === 200) {
            console.log("Success", response?.data);
        }
        return response.data;
...
// For All Notes
export const createAndGetAllNotes = (payload, params) => async (dispatch) => {
    await dispatch(addNote(payload));
    await dispatch(fetchAllNotes(params));
};
export const updateNoteByIdAndGetAllNotes = (payload, params) => async (dispatch) => {
    await dispatch(updateNoteById(payload));
    await dispatch(fetchAllNotes(params));
};
export const deleteNotesByIdAndGetAllNotesTotal = (noteIds, params) => async (dispatch) => {
    await dispatch(deleteNotesById(noteIds));
    await dispatch(fetchAllNotes(params));
};
// For related View
export const createAndGetAllNotesByModuleId = (payload, params, moduleName, moduleId) => async (dispatch) => {
    await dispatch(addNote(payload));
    await dispatch(fetchAllNotesByModuleId({ moduleName, moduleId, params }));
};
export const deleteNotesByIdAndGetAllNotesByModuleId = (noteId, params, moduleName, moduleId) => async (dispatch) => {
...
export const { handleInputChangeReducerNote, resetNote, resetNotes, setEditableMode, } = noteSlice.actions;
