...
// Fetch Audit data
export const fetchAllAuditsByModuleId = createAsyncThunk("audit/fetchAllAuditsByOpportunityId", async ({ moduleName, moduleId, }) => {
    try {
        const config = {
            headers: {
        ...
        const response = await axios.get(`${baseUrl}audit/${moduleName}/${moduleId}`, config);
