...
// Fetch Plan data
export const fetchAllPlans = createAsyncThunk("subscriptions/fetchAllPlans", async () => {
    try {
...
// Create Subscription
export const createSubscription = createAsyncThunk("subscriptions/createSubscription", async (payload) => {
    try {
        const config = {
            headers: {
        ...
        const response = await api.post(`subscription/create-subscription`, payload, config);
        if (response.status === 200) {
            console.log("Success", response?.data);
        }
        return response.data;
...
export const verifySubscription = createAsyncThunk("subscriptions/verifySubscription", async (payload) => {
    try {
        const config = {
            headers: {
                Authorization: `Bearer ${isAccessToken(accessToken) ? accessToken : undefined}`,
...
export const createRequestContactAdmin = createAsyncThunk("subscriptions/createRequestContactAdmin", async (payload) => {
    try {
        const config = {
            headers: {
        ...
        const response = await api.post(`customPlanRequest`, payloadWithOutAccessToken, config);
        if (response.status === 200) {
            console.log("Success", response?.data);
        }
        return response.data;
...
export const fetchSubscriptionByUserId = createAsyncThunk("subscriptions/fetchSubscriptionByUserId", async () => {
    try {
        const config = {
            headers: {
        ...
        const response = await api.get(`subscription/get-active-subscription/${userIdHere}`, config);
        if (response.status === 200) {
            console.log("Success", response?.data);
        }
        return response.data;
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(fetchAllPlans.fulfilled, (state, action) => {
            if (action.payload.data?.plans?.length > 0) {
                const subscriptionPlan = action.payload.data?.plans?.filter((plan) => plan?.planType === "Subscription");
                const trialPlan = action.payload.data?.plans?.filter((plan) => plan?.planType === "Trial");
