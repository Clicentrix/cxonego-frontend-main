...
import {
  Organisation,
  SalesPerson,
} from "../../utilities/common/exportDataTypes/organizationDataTypes";
...
type InitialState = {
  ...
  error: string;
  addOrganisationLoader: boolean;
  getOrganisationLoader: boolean;
...
export const fetchAllSalesPersonByUserId = createAsyncThunk(
  "organisations/fetchAllSalesPersonByUserId",
  async () => {
    try {
      ...
      const response = await axios.get(
        `${baseUrl}users/organization/userslist/${userIdHere}`,
        config
      );
...
export const addOrganisationAndUpdateUserData =
  (organisation: Organisation, user: User) => async (dispatch: AppDispatch) => {
...
export const {
  ...
  resetOrganisation,
  handleOrganisationChangeReducer,
  resetOrganisations,
